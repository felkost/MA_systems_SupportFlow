...
session memory now feeds — `supportflow/router`, `supportflow/docs`,
`supportflow/web_search`.

`scripts/seed_prompts.py` cannot do this: it has been add-only since a
past regression and silently skips any name already seeded, regardless
of whether its content still matches that script's own baseline. Per
that script's own docstring, updating a live prompt on purpose means
calling `Langfuse.create_prompt(...)` directly for that one name — this
script is exactly that, done three times.

Fetches each prompt's *live* `production` text first, never a hardcoded
baseline that could have drifted since it was seeded — inserts the new
block immediately before the existing input block, which every one of
the three prompts already has as their model-facing input marker.
Idempotent: a name whose live text already contains
`{{conversation_history}}` is skipped, so re-running this after a
partial failure is safe.

**`--repair` mode.** This script's first version anchored on the bare
`<customer_message>` opening tag and matched its *prose* mention in
Goals/Constraints instead of the real input section, splicing the
history block into the middle of a sentence in all three live prompts.
`--repair` rebuilds from the version immediately before that damage
(`--from-version`, one per name), applies the corrected insertion, and
publishes the result as a new version. It never edits a version in
place — the corrupted ones stay in Langfuse's history, as any prompt
version does.

Does **not** bump `EXPERIMENT` — that lives in `.env`, which this script
never touches. Bump it by hand *before* running this: changing a
prompt's version without bumping the tag is what caused the Langfuse
quality card to sum two different prompt versions under one tag once
already.
...
# corrupting all three live prompts once already. Matching the tag
# together with its placeholder is unambiguous: it occurs exactly once,
# and only where the input section really is.
...
#
# Measured on two live 24-request runs:
...
def _add_history_block(live_text: str) -> str:
    ...
    ValueError
        ...
        broke all three live prompts once already.
    """
...
# one": version ordering does not reliably identify what was production,
# and these three numbers were checked by hand against the real thing.
