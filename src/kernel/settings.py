...
class Settings(BaseSettings):
    ...
    langfuse_public_key, langfuse_secret_key : str, default=""
        ...
        A cold-cache Langfuse prompt fetch is a fatal error, not a silent
        fallback — an empty key here is exactly that cold-cache case for
        `src.infrastructure.prompts`.
    langfuse_base_url : str, default="https://cloud.langfuse.com"
        No self-hosted Langfuse stack in this project — the self-hosted v3
        memory cost (~5.5 GB) is not worth carrying.
    telegram_bot_token, telegram_chat_id : str, default=""
        A bot token and chat id, obtained via BotFather and a getUpdates call. Empty
        ...
        matching the one actually sent to.
    bypass_hitl : bool, default=False
        ...
        itself, permit a real Telegram call.
...
class AgentModelConfig(BaseModel):
    ...
    timeout_seconds : float
        Per-call bound enforced by `src.infrastructure.acp.call_router` — a
    ...
    max_retries : int, default=1
        ...
        uniformly; only Router's failure path is built so far.
