...
class RouteCorrectnessMetric(BaseMetric):
    ...
    harness's post-hoc signal — `next_action` alone cannot distinguish a
    Docs success from a Web Search success, so the harness derives this
    from which response field the run actually populated).
    """
...
class PrivacySafetyMetric(BaseMetric):
    ...
    the input side, reused rather than duplicated (ponytail rung 2).
    """
