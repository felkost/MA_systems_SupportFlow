"""Golden-dataset case runner (Stage 4 Wave B decision D-B6). Dispatches
each case to one of two paths and returns a populated
...
- The 3 `fault_injection` cases (OAuth error, generic Silpo MCP
  ...
  `graph_nodes.call_docs_agent` replaced at the layer the real D-B3 fix
...
# Stage 4 Wave B decision D-B3: mocked at the graph-node boundary, same
# technique for both — the real fix (docs_a2a_server.py's own except
# branch) is proven separately by tests/interfaces/test_docs_a2a_server.py,
# not re-derived here.
_FAULT_MESSAGES = {
...
def _actual_route(state: SupportFlowState) -> str:
    """Stage 4 Wave B decision D-B1's post-hoc route signal, corrected in
    round 2 of that wave's kickoff review: `next_action` alone cannot
    distinguish a Docs success from a Web Search success (both read
    "respond"), and the three response fields are not mutually exclusive
...
# Stage 4 Wave B decision D-B6: each replacement injects the fault at the
# exact layer D-B3's table names for it — `silpo_unavailable` below
# `call_docs_agent` (inside `run_docs_agent`'s own degrade path),
# `oauth_error`/`timeout` at `call_docs_agent` itself (the signal both
# legitimately cross the A2A hop as, per D-B3's corrected fix).
...
def _bypass_hitl() -> Iterator[None]:
    """Stage 4 Wave B decision D-B9's flags box, closing failure-and-abuse
    row FB3's finding #2 (Lane 4): without this, an escalating golden case
    (guaranteed to exist — the dataset's own required content) blocks on
    `escalation_agent.py`'s interactive confirm prompt the first time this
    runs non-interactively. Scoped with try/finally, never a permanent
    module-level mutation, and deliberately leaves `settings.allow_real_send`
    untouched — two different flags for two different questions
    (decision #19): this only skips the confirm prompt, it never permits a
    real Telegram send.
    """
...
def warm_up() -> None:
    ...
    lazy-loaded on first request, decision #7) before any case that
    actually counts is scored — moved here from
    `scripts/run_golden_dataset_baseline.py`'s own `_warm_up()` (Stage 4
    Wave B Step 4 prerequisite) so `deepeval test run
    tests/test_golden_dataset.py -m eval` gets the same protection a
    baseline script run always had. Best-effort: a throwaway call
    failing here must never fail the whole run.
    """
    with _bypass_hitl():
        for query in _WARMUP_QUERIES:
            try:
                ...
                # was understood (docs/decisions.md #54).
...
def run_case(case: dict[str, Any]) -> LLMTestCase:
    """Stage 4 Wave B decision D-B6: the only branch point — everything
    else about a case (which metrics apply, thresholds) is decided by the
    caller (`tests/test_golden_dataset.py`), not here.
    """
    with _bypass_hitl():
...
# Stage 4 Wave B decision D-B9: THIS is the one place every DeepEval
# metric threshold lives — task §10's own instruction is that thresholds
# are set from the first full baseline run, not asserted in advance, so
# every value below starts as the orientation FLOOR (§10's own wording),
# not a target. After `scripts/run_golden_dataset_baseline.py` produces
# `output/deepeval_baseline.json`, replace these four numbers with the
# measured baseline (never lower than the floor, per §10) and record the
# change as a numbered decision in docs/decisions.md — nowhere else in
# this codebase reads a threshold for these metrics.
...
def _judge_model() -> OpenRouterModel:
    """Stage 4 Wave B decision D-B8: reuse `config/models.yaml`'s existing
    `judge` entry — not part of `load_agent_config`'s `AgentRole` set (it
    has no temperature/timeout/etc. fields), so read directly here.
    """
...
def metrics_for_test_case(test_case: LLMTestCase) -> list[BaseMetric]:
    """Stage 4 Wave B decision D-B1/FB2: `PrivacySafetyMetric` and
    `RouteCorrectnessMetric` apply to every case except a `reject`-expected
    one (asserted directly on `next_action` in the test body instead, per
    D-B1's round-2 fix — there is no response field to derive a route
    signal from on that path). `Support Resolution Quality` (GEval)
    ...
    if actual_route == "docs":
        ...
        # decision #46), so this metric only means what the dataset
        # intends when the actual route is `docs`. Live-confirmed
        # 2026-08-26: attaching it for `web_search` too failed every such
        # case on a provider-name "tool" the dataset never anticipated
        # (docs/decisions.md #58's follow-up).
        #
        ...
        # Correctness; decision #46/D-B7.2 — extra bootstrap tool names
        # in `tools_called` shouldn't fail a case whose `expected_tools`
        # already matched).
        metrics.append(
            ToolCorrectnessMetric(
