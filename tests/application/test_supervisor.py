...
def test_graph_invoke_passes_callbacks_when_tracing_enabled(
    ...
    # `session_id` as `thread_id` — the checkpointer's own continuity
    # key, not a Langfuse concept, so it belongs alongside `callbacks` in
    # this same spy rather than a new test.
